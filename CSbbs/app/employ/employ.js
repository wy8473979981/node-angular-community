//首页的视图控制器
angular.module('CSbbs').controller('employCtrl', function($scope, $http, $rootScope){
	...
	$rootScope.employTopics = [];
	for(var i = 0; i < $rootScope.topics.length; i++){
		if($rootScope.topics[i].block == '招聘'){
			$rootScope.employTopics.push($rootScope.topics[i]);
		}
	}
	...
	$scope.currentPages = [];
	if(($rootScope.employTopics.length)%25 == 0){
		$scope.pageLength = parseInt(($rootScope.employTopics.length)/25);
	}
	else{
		$scope.pageLength = parseInt(($rootScope.employTopics.length)/25) + 1;
	}
	...
	$scope.getPage = function(currentPage){
		...
		if(currentPage != $scope.pageLength){
			$scope.currentPageTopics = $rootScope.employTopics.slice(25 * (currentPage-1), 25 * currentPage);
		}
		//最后一页
		else{
			$scope.currentPageTopics = $rootScope.employTopics.slice(25 * (currentPage-1));
		}
