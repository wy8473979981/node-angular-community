//首页的视图控制器
angular.module('CSbbs').controller('shareCtrl', function($scope, $http, $rootScope){
	...
	$rootScope.shareTopics = [];
	for(var i = 0; i < $rootScope.topics.length; i++){
		if($rootScope.topics[i].block == '分享'){
			$rootScope.shareTopics.push($rootScope.topics[i]);
		}
	}
	...
	$scope.currentPages = [];
	if(($rootScope.shareTopics.length)%25 == 0){
		$scope.pageLength = parseInt(($rootScope.shareTopics.length)/25);
	}
	else{
		$scope.pageLength = parseInt(($rootScope.shareTopics.length)/25) + 1;
	}
	...
	$scope.getPage = function(currentPage){
		...
		if(currentPage != $scope.pageLength){
			$scope.currentPageTopics = $rootScope.shareTopics.slice(25 * (currentPage-1), 25 * currentPage);
		}
		//最后一页
		else{
			$scope.currentPageTopics = $rootScope.shareTopics.slice(25 * (currentPage-1));
		}
