//首页的视图控制器
angular.module('CSbbs').controller('qandaCtrl', function($scope, $http, $rootScope){
	...
	$rootScope.qandaTopics = [];
	for(var i = 0; i < $rootScope.topics.length; i++){
		if($rootScope.topics[i].block == '问答'){
			$rootScope.qandaTopics.push($rootScope.topics[i]);
		}
	}
	...
	$scope.currentPages = [];
	if(($rootScope.qandaTopics.length)%25 == 0){
		$scope.pageLength = parseInt(($rootScope.qandaTopics.length)/25);
	}
	else{
		$scope.pageLength = parseInt(($rootScope.qandaTopics.length)/25) + 1;
	}
	...
	$scope.getPage = function(currentPage){
		...
		if(currentPage != $scope.pageLength){
			$scope.currentPageTopics = $rootScope.qandaTopics.slice(25 * (currentPage-1), 25 * currentPage);
		}
		//最后一页
		else{
			$scope.currentPageTopics = $rootScope.qandaTopics.slice(25 * (currentPage-1));
		}
